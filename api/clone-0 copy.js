...
//////////////// aws live privew /////////
// Serve cloned folders at /preview/:id
app.use('/preview/:id', (req, res, next) => {
  const dir = path.join('/tmp', 'downloads', req.params.id);
  express.static(dir)(req, res, next);
});

app.get('/download/:zipfile', (req, res) => {
  const zipPath = path.join('/tmp', req.params.zipfile);
...
app.post('/api/clone', async (req, res) => {
  ...
  try {
    ...
    ////////////////
      // Zip and serve the fold
      const previewUrl = `http://${req.headers.host}/preview/${timestamp}`;
      output.on('close', () => {
        res.json({
